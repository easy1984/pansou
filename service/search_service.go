...
type SearchResponse struct {
	...
	TVBoxData    *TVBoxCompatibleResponseData `json:"tvbox_data,omitempty" sonic:"tvbox_data,omitempty"` // TVBox兼容数据
}
...
// TVBoxCompatibleLink alist-tvbox 兼容的链接结构
type TVBoxCompatibleLink struct {
	URL       string    `json:"url" sonic:"url"`
	Password   string    `json:"password" sonic:"password"`
	Title      string    `json:"title" sonic:"title"`      // 简洁的标题
	Note       string    `json:"note" sonic:"note"`        // 简洁的备注信息
	Datetime   time.Time `json:"datetime" sonic:"datetime"`
	Source     string    `json:"source,omitempty" sonic:"source,omitempty"` // 数据来源
	Images     []string  `json:"images,omitempty" sonic:"images,omitempty"`   // 图片链接
	LinkType   string    `json:"type,omitempty" sonic:"type,omitempty"`     // 网盘类型（数字编码）
}

// TVBoxCompatibleResponse alist-tvbox 兼容的响应结构
type TVBoxCompatibleResponse struct {
	Code    int                      `json:"code" sonic:"code"`
	Message string                    `json:"message" sonic:"message"`
	Data    TVBoxCompatibleResponseData `json:"data" sonic:"data"`
}

// TVBoxCompatibleResponseData alist-tvbox 兼容的响应数据
type TVBoxCompatibleResponseData struct {
	Total   int                        `json:"total" sonic:"total"`
	Results []TVBoxCompatibleLink      `json:"results" sonic:"results"`
}
