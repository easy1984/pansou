...
// 高清关键词列表
var hdKeywords = []string{"高清", "HD", "1080p", "4K", "2K", "蓝光", "BD", "超清", "超高清"}

// 更新集数正则表达式
var updateRegex = regexp.MustCompile(`(更新到\d+集)`)
...
func (s *SearchService) Search(keyword string, channels []string, concurrency int, forceRefresh bool, resultType string, sourceType string, plugins []string, cloudTypes []string, ext map[string]interface{}) (model.SearchResponse, error) {
	...
	// 1. 过滤出高清内容
	allResults = filterHDResults(allResults)

	// 2. 每个频道只保留最新的一条结果
	allResults = keepLatestPerChannel(allResults)

	// 3. 优化标题：搜索名前置，更新到xx集前置
	allResults = optimizeResultsTitle(allResults, keyword)
...
// isHDContent 检查搜索结果是否包含高清内容
func isHDContent(result model.SearchResult) bool {
	// 检查标题
	title := strings.ToLower(result.Title)
	for _, keyword := range hdKeywords {
	...
	for _, link := range result.Links {
		if link.WorkTitle != "" {
			workTitle := strings.ToLower(link.WorkTitle)
			for _, keyword := range hdKeywords {
				if strings.Contains(workTitle, strings.ToLower(keyword)) {
					return true
				}
			}
		}
...
// filterHDResults 过滤出高清内容
func filterHDResults(results []model.SearchResult) []model.SearchResult {
	var hdResults []model.SearchResult
	for _, result := range results {
		if isHDContent(result) {
			hdResults = append(hdResults, result)
		}
	}
...
// keepLatestPerChannel 每个频道只保留最新的一条结果
func keepLatestPerChannel(results []model.SearchResult) []model.SearchResult {
	// 按频道分组
	channelMap := make(map[string]model.SearchResult)

	for _, result := range results {
		if existing, exists := channelMap[result.Channel]; exists {
			// 如果当前结果更新，则替换
			if result.Datetime.After(existing.Datetime) {
				channelMap[result.Channel] = result
			}
		} else {
			// 新频道，直接添加
			channelMap[result.Channel] = result
	...
	// 转换回切片
	latestResults := make([]model.SearchResult, 0, len(channelMap))
	for _, result := range channelMap {
		latestResults = append(latestResults, result)
	}

	// 按时间排序
	sort.Slice(latestResults, func(i, j int) bool {
		return latestResults[i].Datetime.After(latestResults[j].Datetime)
	})

	return latestResults
}

// optimizeTitle 优化标题：搜索名前置，更新到xx集前置
func optimizeTitle(title, keyword string) string {
	title = strings.TrimSpace(title)

	// 提取更新集数
	updateMatch := updateRegex.FindString(title)
	if updateMatch != "" {
		// 从原标题中移除更新集数
		title = updateRegex.ReplaceAllString(title, "")
		title = strings.TrimSpace(title)
	}
	...
	// 更新集数前置
	if updateMatch != "" {
		newTitle = updateMatch + " " + newTitle
	}

	return strings.TrimSpace(newTitle)
}

// optimizeResultsTitle 优化所有结果的标题
func optimizeResultsTitle(results []model.SearchResult, keyword string) []model.SearchResult {
	for i := range results {
		results[i].Title = optimizeTitle(results[i].Title, keyword)
	}
	return results
}
